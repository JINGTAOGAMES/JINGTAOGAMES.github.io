(function () {
  ...
  var CHARACTERS = [
    ...
    },
    {
      ...
      canMove: true,
      media: {
        move: '/en/img/pets/exusiaiMove-x1.webm',
        interact: '/en/img/pets/exusiaiInteract-x1.webm',
  ...
  function init() {
    ...
    // Scale is anchored at the visual's bottom-center (transform-origin:50% 100%),
    // so extra size grows upward and outward from that point, symmetrically only
    // in the horizontal direction; the feet position stays fixed. Bounds must
    // reflect the actual rendered box, not a naive boxW*scale/boxH*scale.
    ...
    // ---------- multi-state creature behavior (e.g. exusiai: defaultX/move/interact/sleep/relax/special) ----------
    var msState = 'relax';
    ...
    function msEnterMove() {
      ...
      var moveSpeed = 0.5;
      var minDurationMs = 1000; // walk one direction for at least 1s to avoid rapid flip-flopping
      var maxDurationMs = 3500;
      var estFrameMs = 16.7;    // approx per-frame time at ~60fps, used to estimate required room
      ...
      if (canLeft && canRight) {
        ...
        // neither side has enough room for a full 1s walk (viewport too narrow); pick the larger side
    ...
    function msEnterSleep() {
      msState = 'sleep';
      // facing stays as it was while moving; don't touch --bp-face here
    ...
    // ---------- click interaction: hop / play special animation + show text ----------
    ...
    sprite.addEventListener('click', function () {
      ...
      if (isMultistate) {
        if (msState === 'special') return; // non-interruptible, ignore clicks
